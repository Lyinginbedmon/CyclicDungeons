...
public class ModularTrap extends Trap
{
	...
	public void apply(BlockPos min, BlockPos max, ServerWorld world, RoomMetadata meta)
	{
		...
		for(Module component : modules)
		{
			...
			viable.removeIf(p -> !component.isValidRelation(p, componentMap));
	...
	public static record Module(
			Identifier name, 
			int count, 
			BlockPredicate positioning, 
			Optional<Map<Identifier, BlockPos>> relations, 
			BlockState state, 
			...
			Optional<Boolean> isVital)
	{
		public static final Codec<Module> CODEC	= RecordCodecBuilder.create(instance -> instance.group(
				...
				).apply(instance, (name,count,state,inputs,vital) -> 
				{
		...
		{
			return positioning.applyTo(pos, world);
		}
		...
		{
			return relations().orElse(Map.of()).containsKey(name);
		}
		...
		public boolean isValidRelation(BlockPos pos, Map<Identifier, List<BlockPos>> componentMap)
		{
			if(relations.isEmpty())
				return true;
			
			Map<Identifier, BlockPos> relationships = relations.get();
			for(Identifier neighbour : relationships.keySet())
			{
				List<BlockPos> points = componentMap.getOrDefault(neighbour, Lists.newArrayList());
				if(points.isEmpty())
					return false;
				
				BlockPos offset = pos.add(relationships.get(neighbour));
				if(points.stream().noneMatch(p -> p.getManhattanDistance(offset) == 0))
					return false;
			}
			return true;
		}
		...
		public static class Builder
		{
			...
			private Map<Identifier, BlockPos> relations = new HashMap<>(); 
			...
			public Builder relation(Identifier id, BlockPos relation)
			{

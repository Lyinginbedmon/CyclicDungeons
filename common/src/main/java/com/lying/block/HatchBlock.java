...
public class HatchBlock extends AbstractTrapActorBlock
{
	...
	public Vec3d wireRenderOffset(BlockState state)
	{
		...
		return new Vec3d(face.getOffsetX(), 0, face.getOffsetZ()).multiply(0.45).negate().add(0D, 0.5D, 0D);
	}
	
	public BlockState getPlacementState(ItemPlacementContext ctx)
	{
		Direction facing = ctx.getSide();
		return facing.getAxis() == Axis.Y ? null :
				getDefaultState()
				.with(FACING, facing)
				.with(INTERSTITIAL, isInterstitial(facing, ctx.getWorld(), ctx.getBlockPos()));
	}
